...
const LISTEN_HOST = process.env.LAN_GATE_HOST || '127.0.0.1'
...
const TRUSTED_PROXIES = String(process.env.LAN_GATE_TRUSTED_PROXIES || '').split(',').map(function (s) { return s.trim() }).filter(Boolean)
...
webpush.setVapidDetails(process.env.LAN_GATE_VAPID_SUBJECT || 'mailto:admin@localhost', state.vapid.publicKey, state.vapid.privateKey)
...
function deliverPush(subscription, payload, cb) {
  ...
  var pReq = mod.request({ hostname: u.hostname, port: u.port, path: u.pathname + u.search, method: details.method, headers: details.headers }, function (pRes) {
    ...
    pRes.on('end', function () {
      ...
      var err = new Error('push status ' + code); err.statusCode = code; cb(err)
    })
  })
  ...
  var upstream = http.request({ host: TARGET_HOST, port: TARGET_PORT, method: req.method, path: req.url, headers: headers }, function (upRes) {
    ...
    if (isHtml) {
      ...
      upRes.on('end', function () {
        ...
        try {
          var html = injectHtml(Buffer.concat(chunks).toString('utf8'), kind)
...
var server = http.createServer(function (req, res) {
  ...
  if (overRate(client.ip)) { sendHtml(req, res, 429, rateLimitPage(client.ip), { 'Retry-After': '60' }); return }
...
server.on('upgrade', function (req, socket, head) {
  var client = resolveClient(req)
  if (overRate(client.ip)) { try { socket.end('HTTP/1.1 429 Too Many Requests\r\nConnection: close\r\n\r\n') } catch (e) {} return }
  var device = deviceForReq(req)
  var ok = isLocalDirect(req, client) || device !== undefined
